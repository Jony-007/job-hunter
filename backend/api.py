...
async def tailor_resume(body: TailorResumeRequest, user: dict = Depends(get_current_user)) -> Dict[str, Any]:
    ...
    system_prompt = (
        ...
        "1. Evaluate the user's current resume against the job description to calculate an initial/current ATS match score (an integer from 0 to 100).\n"
        "2. Tailor the resume to match the job description, optimizing technical skills and achievements.\n"
        "3. Evaluate the newly tailored resume to calculate the optimized ATS match score (an integer from 0 to 100).\n"
    ...
    if not deepseek_key and not anthropic_key:
        ...
        fallback_resume = (
            ...
            f"## Tailored Professional Accomplishments\n"
            f"### Infrastructure Support Engineer | Specialized Contributions\n"
            f"- Integrated automated PowerShell scripting arrays to streamline administration, boosting active support efficiency.\n"
            f"- Designed and deployed core cloud systems within M365 and Azure environments matching **{job_company}** standards.\n"
            f"- Managed active troubleshooting diagnostics across enterprise endpoints, optimizing Active Directory domains.\n\n"
            f"---\n"
